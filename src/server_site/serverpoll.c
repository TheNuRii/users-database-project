...
void fsm_reply_hello(clientstate_t *client, dbproto_hdr_t *hdr) {
    hdr->type = (dbproto_type_e)htonl(MSG_HELLO_RESP);
    hdr->len = htons(1);
...
void fsm_reply_add(clientstate_t *client, dbproto_hdr_t *hdr) {
    hdr->type = (dbproto_type_e)htonl(MSG_EMPLOYEE_ADD_RESP);
    hdr->len = htons(0);
...
void fsm_reply_hello_err(clientstate_t *client, dbproto_hdr_t *hdr) {
    hdr->type = (dbproto_type_e)htonl(MSG_ERROR);
    hdr->len = htons(0);
...
void send_employees(struct dbheader_t *dbhdr, struct employee_t **employeeptr, clientstate_t *client) {
    ...
    hdr->type = (dbproto_type_e)htonl(MSG_EMPLOYEE_LIST_RESP);
    hdr->len = htons(dbhdr->count);

    write(client->fd, hdr, sizeof(dbproto_hdr_t));

    dbproto_emplyee_list_resp *employee = (dbproto_emplyee_list_resp*)&hdr[1];
    ...
    for (; i < dbhdr->count; i++) {
        ...
        write(client->fd, employee, sizeof(dbproto_emplyee_list_resp));
    }
}
